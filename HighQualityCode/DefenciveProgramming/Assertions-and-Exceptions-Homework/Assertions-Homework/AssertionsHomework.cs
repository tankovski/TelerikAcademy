...
class AssertionsHomework
{
    ...
    {
        Debug.Assert(startIndex >= 0 && startIndex<arr.Length, "Start index is out of the array bounds!");
        Debug.Assert(endIndex >= 0 && endIndex < arr.Length, "End index is out of the array bounds!");
        ...
        // check if is this realy the smallest element
        for (int i = startIndex + 1; i < endIndex; i++)
        {
    ...
    private static int BinarySearch<T>(T[] arr, T value, int startIndex, int endIndex) 
        where T : IComparable<T>
    {
        Debug.Assert(startIndex >= 0 && startIndex < arr.Length, "Start is out of the array bounds!");
        Debug.Assert(endIndex >= 0 && endIndex < arr.Length, "End index is out of the array bounds!");
        Debug.Assert(startIndex < endIndex, "Start index must be smaller than end index!");
